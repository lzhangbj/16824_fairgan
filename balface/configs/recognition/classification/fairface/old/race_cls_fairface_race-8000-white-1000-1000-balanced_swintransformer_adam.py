gpus=(0,1,2,3,5,6,7)

data = dict(
    train=dict(
        dataset="cls_fairface",
        root="./datasets/FairFace/images",
        n_classes=[7,],
        face_bbox_txt="./datasets/FairFace/bbox/fairface_train_bbox.txt",
        face_label_txt="./datasets/FairFace/labels/race/fairface_train_race-8000-white-1000-1000-balanced_label.txt",
        size=224,
        samples_per_gpu=128,
        workers_per_gpu=8,
        mode='train'
    ),
    val=dict(
        dataset="cls_fairface",
        root="./datasets/FairFace/images",
        size=224,
        n_classes=[7,],
        face_bbox_txt="./datasets/FairFace/bbox/fairface_val_bbox.txt",
        face_label_txt="./datasets/FairFace/labels/all/fairface_val_all_label.txt",
        samples_per_gpu=128,
        workers_per_gpu=8,
        mode='val'
    )
)

model = dict(
    name='cls_recognizer',
    backbone=dict(
        name="torch_swintransformer"
    ),
    head=dict(
        name="multiclassifier_head",
        n_classes=[7,],
        input_dim=96*8,
        hidden_dims=[]
    ),
    use_weight=False
)


trainer = dict(
    name='cls',
    max_epochs=40,
    save_freq=2,
    print_freq=10,
    val_freq=2,
    loss_weight='none'
)

optimizer = dict(
    name='adam',
    lr=1e-4,
    weight_decay=5e-4
)

lr_scheduler = dict(
    name='multisteplrwwarmup',
    milestones=[24, 34],
    warmup_epoch=2,
    warmup_lr=0.00001
)

find_unused_parameters = False

resume_from = None # 'work_dirs/race_cls_fairface_race-8000-white-5000-500_resnet34_adam/epoch_40.pth'
load_backbone = None  # 'work_dirs/ssp_fakemake-100000_resnet34_adam/epoch_40.pth'