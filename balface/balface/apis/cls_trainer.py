...
class CLSTrainer:
	def __init__(self, cfg):
		...
		self.logger.info(f"########## training with {len(train_dataset)} data ###########")
		...
		self.cls_loss_weight /= torch.tensor(self.cfg.trainer.sample_ratio).cuda()
	...
	def run(self):
		...
		#####   test on best epoch #####
		self.logger.info("############ best epoch evaluation ############\n###################################")
		best_ckpt = f"{self.cfg.work_dir}/best_model.pth"
		self.model.eval()
		self.load_from(best_ckpt)
		top1s, cls_recalls, cls_precs = self.test(bias_validate=False)
		output = f"{best_epoch} Test Results: \n"
		for i in range(n_classifier):
			output += f'attr_{i + 1} Prec@1 {top1s[i]:.3f} '
			if i == 0:
				output += f'; Eval Prec@1 {best_top1:.3f}'
			out_cls_recalls = 'Test Class Recalls: \n%s' % (
				(np.array2string(cls_recalls[i], separator='\n', formatter={'float_kind': lambda x: "%.3f" % x})))
			output += out_cls_recalls
			output += "\n"

			out_cls_precs = 'Test Class Precs: \n%s' % (
		...
		self.logger.info("############ last epoch evaluation ############\n###################################")
		best_ckpt = f"{self.cfg.work_dir}/latest.pth"
		self.model.eval()
		self.load_from(best_ckpt)
		top1s, cls_recalls, cls_precs = self.test(bias_validate=False)
		output = f"{best_epoch} Test Results: \n"
	...
	def train(self):
		...
		for i, (inputs, target, weights) in enumerate(self.train_loader):
		...
		self.logger.info(
			f'$$$$ epoch avg loss labeled: {labeled_losses[0].avg} $$$')
