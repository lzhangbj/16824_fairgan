...
class ClusterTrainer:
    ...
    def run(self):
        ...
        for self.epoch in range(init_epoch, max_epochs+1):
            ...
            # print(torch.norm(self.model.module.clusters, dim=1))
        ...
        if dist.get_rank() == 0:
            top1s, cls_accs, bias_accs = self.validate(bias_validate=False)
        #     output = "Eval Results: \n"
        #     if top1s[0] > best_top1:
        #         best_top1 = top1s[0]
        #         best_epoch = self.epoch
    ...
    def train(self):
        ...
        n_classifier = len(self.cfg.data.labeled_train.n_classes)
        labeled_cluster_losses = AverageMeter('Loss', ':.4e')
        weak_cluster_losses = AverageMeter('Loss', ':.4e')
        strong_cluster_losses = AverageMeter('Loss', ':.4e')
        cluster_reg_losses = AverageMeter('Loss', ':.4e')
        cluster_margin_losses = AverageMeter('Loss', ':.4e')
        cluster_std_losses = AverageMeter('Loss', ':.4e')
        ...
        for i, unlabeled_batch in enumerate(self.unlabeled_dataloader):
            weak_unlabeled, strong_unlabeled = unlabeled_batch[:2]
            try:
            ...
            weak_unlabeled = weak_unlabeled.cuda()
            strong_unlabeled = strong_unlabeled.cuda()

            output, loss_dict = self.model(labeled_inputs, weak_unlabeled, strong_unlabeled, target)
            ...
            for key in loss_dict:
                ...
                if 'cluster_reg' in key:
                    loss_dict[key] *= self.cfg.trainer.cluster_reg_weight
                if 'cluster_margin' in key:
                    loss_dict[key] *= self.cfg.trainer.cluster_margin_weight
                if 'std' in key:
                    loss_dict[key] *= self.cfg.trainer.cluster_std_weight
                if 'weak' in key:
                    loss_dict[key] *= self.cfg.trainer.weak_cluster_weight
                if 'strong' in key:
                    loss_dict[key] *= self.cfg.trainer.strong_cluster_weight

            loss = sum([t.mean() for t in loss_dict.values()])

            acc1, acc5 = accuracy(output[0], target[:, 0], topk=(1, 5))
            losses.update(loss.item(), labeled_inputs.size(0))
            top1.update(acc1[0], labeled_inputs.size(0))

            for j in range(n_classifier):
                labeled_losses[j].update(loss_dict[f'cls_{j+1}_loss'].item(), labeled_inputs.size(0))
                try:
                    unlabeled_losses[j].update(loss_dict[f"ssl_{j+1}_loss"].item(), weak_unlabeled.size(0))
                except:
                    pass

            labeled_cluster_losses.update(loss_dict[f'labeled_center_loss'].item(), labeled_inputs.size(0))
            weak_cluster_losses.update(loss_dict[f'weak_center_loss'].item(), weak_unlabeled.size(0))
            strong_cluster_losses.update(loss_dict[f'strong_center_loss'].item(), weak_unlabeled.size(0))
            cluster_reg_losses.update(loss_dict[f'cluster_reg_loss'].item(), labeled_inputs.size(0))
            cluster_margin_losses.update(loss_dict[f'cluster_margin_loss'].item(), labeled_inputs.size(0))
            cluster_std_losses.update(loss_dict[f'cluster_std_loss'].item(), labeled_inputs.size(0))
            ...
            if i % self.cfg.trainer.print_freq == 0:
                ...
                for j in range(n_classifier):
                    output += f"cls_{j+1} = {labeled_losses[j].val:.4f}\t"
                    try:
                        output += f"ssl_{j+1} = / {unlabeled_losses[j].val: .4f}\t"
                    except:
                        pass
                    output += f"center = {labeled_cluster_losses.val:.4f}\t"
                    output += f"weak = {weak_cluster_losses.val:.4f}\t"
                    output += f"strong = {strong_cluster_losses.val:.4f}\t"
                    output += f"reg = {cluster_reg_losses.val:.4f}\t"
                    output += f"margin = {cluster_margin_losses.val:.4f}\t"
                    output += f"std = {cluster_std_losses.val:.4f}\t"
                self.logger.info(output)
    ...
    def validate(self, record=True, bias_validate=False, save_embeddings=False):
        batch_time = AverageMeter('Time', ':6.3f')
        ...
        with torch.no_grad():
            ...
            for i, (inputs, target) in enumerate(self.val_loader):
                ...
                if save_embeddings:
                    ...
                    pseudo_target = torch.argmax(output[0], dim=1).cpu().numpy()
                    pseudo_targets_list.append(pseudo_target)
            ...
            if save_embeddings:
                ...
                pseudo_targets = np.concatenate(pseudo_targets_list)
                np.save('cluster_fairface-val_embeddings', embeddings)
                np.save('cluster_fairface-val_labels', targets)
                np.save('cluster_fairface-val_pseudo-labels', pseudo_targets)
                cls_centers = self.model.module.clusters.data.cpu().numpy()
                np.save('cluster_centers', cls_centers)
