...
class CutmixTrainer:
    ...
    def run(self):
        ...
        for self.epoch in range(init_epoch, max_epochs+1):
            self.train_loader.dataset.max_coeff = 0.5 # min(0.5, 0.1 + self.epoch / float(self.cfg.lr_scheduler.milestones[0]) * 0.4)
            self.train()
    ...
    def update_loss_weight_v2(self):
        ...
        with torch.no_grad():
            for i, (inputs, target) in enumerate(self.train_loader):
                inputs = inputs.cuda()
                target = target.cuda()
                output, loss_dict = self.model(inputs, target.unsqueeze(1))
                assert len(output) == 1
                output = output[0]
            ...
            for i in range(n_class):
                ...
                weights = labels[:, i]
                class_prob = probs[:, i]
                mean_prob.append((class_prob * weights).sum() / weights.sum())
                class_var = vars[:, i]
                mean_var.append((class_var * weights).sum() / weights.sum())
            ...
            sample_num = labels.sum(dim=0)
            sample_ratio = sample_num / sample_num.sum()

            sample_ratio = sample_ratio.cuda() ** 1.0
            sample_ratio.requires_grad = False
    ...
    def update_loss_weight_v3(self):
        ...
        with torch.no_grad():
            for i, (inputs, target) in enumerate(self.train_loader):
                ...
                prob = torch.softmax(output, 1)
                var = prob * (1-prob)
                embeddings_norms = (1-prob) * embeddings_norms.unsqueeze(1)
            ...
            for i in range(n_class):
                ...
                class_var = vars[:, i]
                mean_var.append((class_var * weights).sum() / weights.sum())
                mean_norm.append((embeddings_norms[:, i] * weights).sum() / weights.sum())
    ...
    def train(self):
        ...
        for i, (inputs, target) in enumerate(self.train_loader):
            ...
            acc1 = accuracy(output[0], torch.argmax(target[:, 0], dim=1), topk=(1, ))[0]
