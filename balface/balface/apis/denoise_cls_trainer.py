...
from balface.utils import get_root_logger, AverageMeter, accuracy, sync_tensor_across_gpus, SelfAdaptiveTrainingCE
...
class DenoiseCLSTrainer:
	def __init__(self, cfg):
		...
		try:
			momentum = self.cfg.trainer.denoise_momentum
		except:
			momentum = 0.8
		self.loss_func = SelfAdaptiveTrainingCE(train_dataset.get_labels(), num_classes=n_class, momentum=momentum)
	...
	def update_loss_weights(self):
		...
		with torch.no_grad():
			...
			cls_loss_weight = cls_loss_weight / cls_loss_weight.sum() * n_class + 1e-4
	...
	def train(self):
		...
		# self.cls_loss_weight = self.cls_loss_weight / self.cls_loss_weight.sum() * 4.
		end = time.time()
		for i, (inputs, target, weights, indices) in enumerate(self.train_loader):
			...
			output, _ = self.model(inputs, target, self.cls_loss_weight)
			logits = output[-1]
			
			global_logits = sync_tensor_across_gpus(logits)
			global_indices = sync_tensor_across_gpus(indices)
			self.loss_func.update_labels(global_logits, global_indices)

			loss_dict = {'cls_1_loss': self.loss_func(logits, indices)}
			
			for key in loss_dict:
				loss_dict[key] *= weights
